...
function addToCartFromBorders(productId, category) {
  ...
  let productData = border.find((p) => p.id === productId);
  if (!productData) {
...
function renderCart() {
  ...
  cartBox.innerHTML = "";
  basket.forEach((prod) => {
    let productInfo = border.find((item) => item.id === prod.id);
...
function createOrder() {
  ...
  if (!address) {
    ...
    document.getElementById("addressError").style.color = "#CC0000";
    return;
  }
