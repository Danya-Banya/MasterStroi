...
let selectedAddress = "";
let map = L.map("map").setView([56.0355, 35.9586], 13);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (mapContainer) {
    ...
    // let marker = L.marker([56.0355, 35.9586]).addTo(map);
    // marker.bindPopup("Волоколамск").openPopup();
  }
});
...
function addToCartFromGutters(productId, category) {
  ...
  let productData = gutters.find((p) => p.id === productId);
  if (!productData) {
...
function renderCart() {
  ...
  cartBox.innerHTML = "";
  basket.forEach((prod) => {
    let productInfo = gutters.find((item) => item.id === prod.id);
