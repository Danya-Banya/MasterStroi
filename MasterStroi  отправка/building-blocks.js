...
function addToCartFromBlocks(productId, category) {
  ...
  let productData = blocks.find((p) => p.id === productId);
  if (!productData) {
...
function renderCart() {
  ...
  cartBox.innerHTML = "";
  basket.forEach((prod) => {
    let productInfo = blocks.find((item) => item.id === prod.id);
...
function createOrder() {
  ...
  if (!deliveryDate) {
    dateError.textContent = "Выберите дату доставки";
    dateError.style.color = "#CC0000";
    return;
  }

  if (!address) {
    addressError.textContent = "Укажите на карте адрес доставки";
    addressError.style.color = "#CC0000";
    return;
  }
